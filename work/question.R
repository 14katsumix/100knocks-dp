...
商品カテゴリごとの販売金額の合計を集計せよ
各商品カテゴリごとの売上金額を集計し、売上金額がゼロのカテゴリも表示するようにしてください。
group_byを使用してカテゴリごとに集計し、completeを使用して売上金額がゼロのカテゴリも含めて表示してください。
...
商品カテゴリ別の累積売上金額を計算せよ
各商品カテゴリについて、販売日順に累積売上金額を求めてください。
この際、cumsum()を用いて計算を行い、カテゴリごとに累積金額がわかるようにしてください。
...
# cumany() ------------
特定のカテゴリの商品を購入したことがある顧客を判定せよ
各顧客について、指定されたカテゴリ（例: 大カテゴリコードが01）の商品を一度でも購入したかどうかを判定してください。
...
商品の単価を分位数でカテゴリ化せよ
商品の単価 (unit_price) を基準に、ntileを使って5つのカテゴリ（五分位数）に分けてください。
各カテゴリに属する商品数を集計し、カテゴリごとの単価範囲を出力してください。
