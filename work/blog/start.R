...
d %>% head(5)
d = query %>% my_select(con, params = list(20200101))
d %>% head(5)
...
db_result %>% my_sql_render(con = simulate_mysql(), subquery = TRUE)
db_result %>% my_sql_render(con = simulate_mysql(), subquery = FALSE)
